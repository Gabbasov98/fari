...
function renderHeaderCartCount() {
    if(cartItems.length === 0){
        $(".header-action__btn--cart").html("")
        return
    }

    $(".header-action__btn--cart").html(`
        <span class="_pin">${cartItems.length}</span>
    `)
}
...
function catalogItem() {
    let id = +$(".catalog-item").attr("data-id")
    let isAdded = cartItems.filter(item => item.id === id)

    if(isAdded.length!==0){
        $(".catalog-item").addClass("_added")
    } else{
        $(".catalog-item").removeClass("_added")
    }

    $(".product-cart__btn").click(function () {
        if($(".catalog-item").hasClass("_added")){
            $(".catalog-item").removeClass("_added")
            removeFromCart(id)
        } else{
            $(".catalog-item").addClass("_added")
...
function initYandexMap () {
    ...
    script.src = 'https://api-maps.yandex.ru/2.1/?lang=ru_RU&amp;apikey=9133eb49-3a97-4b2d-90be-6b80a97a5036';
